...
function solve(
    ...
    b̄ = @pipe b |> to_standard_form(Ψ, _)
    L1, L2, κ  = Lcurve_functions(Ψ, b̄, Ψ.L*x₀)
...
function gcv_svd(Ψ::RegularizationProblem, b::AbstractVector, λ::AbstractFloat)
    F = Ψ.F̄
    n, p = size(F.U)
    z = F.U' * b
    nλ = λ^2.0
    ...
    return n * (norm(b)^2.0 - norm(z)^2.0 + s1) / (n - p + s2)^2.0
end
...
function Lcurve_functions(Ψ::RegularizationProblem, b̄::AbstractVector, x̄₀::AbstractVector)
    ...
    η⁰(λ::AbstractFloat)  = (log.(L2.(λ) .^ 2))[1]
    ρ⁰(λ::AbstractFloat)  = (log.(L1.(λ) .^ 2))[1]
    ...
    function κ(λ::AbstractFloat)  
        ...
        η  = η⁰(λ)
        ρ  = ρ⁰(λ)
        return 1.0 - 2.0*η*ρ*(λ^2.0*nᵖ*ρ + 2.0*λ*η*ρ + λ^4.0*η*nᵖ) / 
